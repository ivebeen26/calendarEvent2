    ...
    <div class="row">
      <div class="col-md-5">
        <div class="form-group">
          ...
          <input type="text" @keyup="reEvaluate" v-model="event" class="form-control" placeholder="Insert Event" id="event">
        </div>
            ...
            <tbody>
              <div v-for="(date, index) in dates" :key="index"> 
                <tr v-for="(td, index) in add_rows(date)" :key="index">
                  <td style="width:50vw">{{ td.day }}</td>
                  <td style="width:50vw">{{ td.day_name }}</td>
                  <td style="width:50vw">{{ td.description }}</td>
                </tr><br/><hr/>
              </div>
            </tbody>
          </table>
  ...
  export default {
    ...
    methods: {
      ...
      addEvent() {
        ...
        let days  = this.getCheckboxFilters()
        return this.$http.post(`api/addEvent`, {event, from, to, days})
      },
      getCheckboxFilters(){
      ...
      add_rows(date) {
        ...
        while (theDate <= endDate) {
          ...
          theDate.setDate(theDate.getDate() + 1)  //increment each day
        }
